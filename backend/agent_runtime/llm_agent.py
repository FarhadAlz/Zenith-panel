...
# When a Dashboard (dashboard.py) is active, tool-call lines and status
# updates are pushed into the live view instead of printed with colors.py.
# Defaults to None so importing/running this module without a dashboard
# (e.g. the test suite, or a plain-stdout invocation) behaves exactly as
# before.
_ui = None


def set_ui(ui):
    """Register the active Dashboard instance. Pass None to fall back to
    plain colors.py stdout printing."""
    global _ui
    _ui = ui


# Tools whose result tells us whether the site itself is currently up —
# used to drive the dashboard's "SITE STATUS" cell as the investigation
# progresses, without adding any new tool calls of our own.
_SITE_STATUS_TOOLS = {"http_check", "verify_http_check"}


def _update_site_status_from_result(tool_name: str, result: dict):
    if _ui is None or tool_name not in _SITE_STATUS_TOOLS:
        return
    status_code = result.get("status_code")
    ok = bool(result.get("ok"))
    if status_code:
        detail = f"HTTP {status_code} · {result.get('elapsed_ms', '?')}ms"
    else:
        detail = result.get("error") or "no response"
    _ui.set_site_status(ok, detail)
...
def execute_tool(tool_name: str, tool_input: dict, auto_approve: bool = False) -> dict:
    ...
    if fn is None:
        ...
        if _ui is not None:
            _ui.log_tool(tool_name, cls, False, result["error"])
        else:
    ...
    if cls == "YELLOW":
        ...
        if precheck_error:
            ...
            if _ui is not None:
                _ui.log_tool(tool_name, cls, False, precheck_error)
            else:
                print(colors.tool_call_line(tool_name, cls, False, precheck_error))
            return result

        if _ui is not None:
            _ui.set_agent_status("waiting_approval")
        ...
        if not approved:
            ...
            safety.log_tool_call(tool_name, tool_input, result)
            if _ui is not None:
                _ui.log_tool(tool_name, cls, False, "denied")
            else:
    ...
    safety.log_tool_call(tool_name, tool_input, result)
    detail = _result_detail(tool_name, result)
    _update_site_status_from_result(tool_name, result)
    if _ui is not None:
        _ui.log_tool(tool_name, cls, result.get("ok", False), detail)
    else:
        print(colors.tool_call_line(tool_name, cls, result.get("ok", False), detail))
