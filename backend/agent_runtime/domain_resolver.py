...
def list_domains() -> dict:
    """
    Enumerate every domain (and its subdomains, best-effort) hosted on
    this server under the DirectAdmin layout, purely by reading
    directory names under HOME_ROOT — the same filesystem boundary
    every other read-only lookup in this module already trusts.

    Read-only, additive, and intentionally dumb: it does not call
    resolve_domain() for each entry (that would be redundant work for
    a listing endpoint), it just reports what glob() sees. Callers that
    need the full webroot/log_paths detail for one specific domain
    should still call resolve_domain(domain) for that domain.
    """

    results = []

    pattern = os.path.join(HOME_ROOT, "*", "domains", "*")

    for domain_dir in sorted(glob.glob(pattern)):
        ...
        webroot = os.path.join(domain_dir, "public_html")

        subdomains = []
        if os.path.isdir(webroot):
            for entry in sorted(os.listdir(webroot)):
                entry_path = os.path.join(webroot, entry)
                if os.path.isdir(entry_path) and _DOMAIN_LABEL_RE.match(entry):
                    subdomains.append(f"{entry}.{domain_name}")

        results.append(
            {
                "domain": domain_name,
                "owner_user": owner,
                "webroot": webroot if os.path.isdir(webroot) else None,
